...
実行するにあたって、maincode.pyを実行するのではなくstart.batを実行してコマンド経由でmaincode.pyを実行してください。
start.batの中身はただmaincode.pyを実行し、maincode.pyの処理が完了後にバッチも終了するというものだけです。
このソフトウェアはgithubの
...
Modpackを入れる場合はmodsなどのフォルダーやファイルをを直で入れる、mod/shaderpacksのみを追加、構成を作成の場合はjarやzip形式などのものをいれるだけです。
